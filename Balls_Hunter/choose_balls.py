...
def ask_color_order():
    ...
    print("=" * 50)
    print("Choisis l'ordre des 4 balles à toucher.")
    print("Codes : g = Green, p = Purple, r = Red, t = Turquoise\n")

    while len(order) < 4:
        idx = len(order) + 1
        s = input(f"Couleur #{idx} (g/p/r/t) : ").strip().lower()
        if s not in mapping:
            print("  → Entrée invalide, recommence (g, p, r, t seulement).")
            continue
        color_name = mapping[s]
        if color_name in order:
            print("  → Cette couleur est déjà choisie, prends-en une autre.")
            continue
        order.append(color_name)

    print("\nOrdre choisi :", " → ".join(order))
    input("Appuie sur Entrée pour démarrer la mission...")
    return order
...
# Récupérer l'ordre des phases
phases = ask_color_order()

print("=" * 50)
print("MISSION CHASSEUR DE BALLES – PARAMÉTRABLE")
print("=" * 50)
print("Ordre :", " → ".join(phases), "\n")
...
try:
    ...
    while True:
        ...
        os.system('clear')
        print("=" * 50)
        print(f"CHASSEUR DE BALLES – PHASE {phase} ({phase_index+1}/4)")
        print("=" * 50)
        ...
        if ball is None:
            ...
            if abs(dx) > TOLERANCE_X_PIXELS:
                ...
                if dist > DISTANCE_CIBLE_CM + TOLERANCE_DISTANCE_CM:
                    ...
                    # Recul sauf après la dernière balle
                    if phase_index < len(phases) - 1:
                        print(f"→ Action : RECUL APRÈS POUSSÉE (balle {couleur})")
                        reculer_asservi(duree_s=2 * PUSH_DURATION_S)
                        print(f"\n✓ Balle {couleur} poussée, robot reculé.")
                        phase_index += 1
                        print(f"→ Passage à la recherche de la balle suivante : {phases[phase_index]}...")
                    else:
                        stop()
                        print("\n✓ Dernière balle poussée, mission complète ✅")
...
#Ajouter si possible un paramètre permettant au robot de reculé si jamais il a fait deux tours complets
#Cela permettra de contrer les problèmes si le robot est trop collé a une balle et ne la detecte plus