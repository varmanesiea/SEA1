...
# Plage HSV pour la BALLE turquoise (à ajuster si besoin)
# H ∈ [0,179], S,V ∈ [0,255] en HSV OpenCV.[web:10][web:11]
...
def detect_turquoise_ball(hsv_image):
...
print("=" * 50)
print("Recherche d’une balle JAUNE, centrage puis arrêt.\n")
...
try:
    ...
    while True:
        ...
        turquoise_ball = detect_turquoise_ball(hsv)
        ...
        if turquoise_ball is None:
            print("\n✗ Aucune balle turquoise détectée")
            print("→ Action : recherche, rotation lente à gauche")
            tourner_gauche_continu(duree_s=0.15, vitesse=SPEED_TURN)

        else:
            bx = turquoise_ball['x']
            dist = turquoise_ball['distance']
            dx = bx - center_x

            print(f"\n✓ Balle turquoise détectée")
