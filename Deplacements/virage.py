...
# VIRAGE 90° DROITE parfait
TICKS_VIRAGE_GAUCHE  = 10
TICKS_VIRAGE_DROITE  = -10
PWM_VIRAGE = 80
...
def virage_droite_pwm():
    pi.write(FR, 0)
    pi.write(BL, 0)
    pi.set_PWM_dutycycle(FL, PWM_VIRAGE)
    pi.set_PWM_dutycycle(BR, PWM_VIRAGE)

print("z : approche 10 cm + virage 90° DROITE précis, q : quitter\n")
...
try:
    while True:
        ...
        if ch == 'z':
            ...
            print("🔄 PHASE 2 : Virage 90° DROITE...")
            
            count_left = 0
            count_right = 0
            
            while count_left < TICKS_VIRAGE_GAUCHE or abs(count_right) < abs(TICKS_VIRAGE_DROITE):
                virage_droite_pwm()
                time.sleep(PERIOD)

            stop()
            print(f"  Virage OK : G={count_left}, D={count_right}")
