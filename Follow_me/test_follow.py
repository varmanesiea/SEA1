...
# =======================
#  Rotations continues (plus rapides)
# =======================

def tourner_gauche_continu(duree_s=0.25, vitesse=120):
    """Tourner à gauche en activant les deux moteurs (un avant, un arrière)."""
    t0 = time.time()
...
def tourner_droite_continu(duree_s=0.25, vitesse=120):
    """Tourner à droite en activant les deux moteurs (un avant, un arrière)."""
    t0 = time.time()
    while time.time() - t0 < duree_s:
...
def suivre_tag_avancer(duree_s=0.3):
    """
    Avancer tout droit pendant duree_s secondes avec asservissement encodeurs.
    Ne touche pas à la caméra.
    """
...
try:
    while True:
        ...
        if markers:
            ...
            # 1) orientation : rotation continue des deux moteurs
            ...
            # 2) distance : petits pas d’avance fluide
