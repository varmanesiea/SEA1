...
try:
    ...
    while True:
        ...
        if markers:
            ...
            # si la perte est récente (< 1s) : on ne bouge pas
            if now - last_seen_time < 1.0:
                print("→ Attente marge d'erreur (1 s)")
            else:
                print("→ Scan léger droite puis gauche")

                # petit coup à droite
                tourner_droite_continu(duree_s=0.05, vitesse=SPEED_TURN)
                time.sleep(0.05)

                image = picam2.capture_array()
                gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                gray_image = cv2.rotate(gray_image, cv2.ROTATE_180)
                markers_scan = extract_markers(gray_image)

                if not markers_scan:
                    # petit coup à gauche (retour + un peu)
